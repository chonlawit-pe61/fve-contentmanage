...
<div class="card shadow shadow-lg">
    <form action="<?= base_url('admin/publish/savePublishEducationalDevelopment') ?>" method="POST" autocomplete="off" enctype="multipart/form-data" class="once-only">
        <input type="hidden" name="publish_educational_development_id" value="<?= (@$PublishEducationalDevelopment['publish_educational_development_id']) ? $PublishEducationalDevelopment['publish_educational_development_id'] : '' ?>">
        <div class="card-body">
            <div class="row">
                    ...
                    <nav class="py-2" aria-label="breadcrumb" style="--bs-breadcrumb-divider: '/'">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">
                                <?= (@$PublishEducationalDevelopment['publish_educational_development_id']) ? 'แก้ไขข้อมูล' : 'เพิ่มข้อมูล' ?>
                            </li>
                        </ol>
                    </nav>
                </div>
            ...
            <div class="row justify-content-center my-2">
                <div class="col-md-10 my-2">
                    ...
                    <select name="data_year" class="form-select" aria-label="Default select example">
                        <?php for ($year = $startYear; $year <= $endYear; $year++): ?>
                            <option <?php echo @$PublishEducationalDevelopment['data_year'] == $year ? 'selected' : '' ?> value="<?= $year ?>">
                                <?= $year + 543 ?>
                            </option>
                        <?php endfor; ?>
                    </select>
                ...
                <div class="col-md-10 my-2">
                    ...
                    <?php
                    if (!empty(@$PublishEducationalDevelopment['publish_educational_development_file'])) {
                    ?>
                        <a class="btn btn-primary mt-3" href="<?= base_url(@$PublishEducationalDevelopment['publish_educational_development_file']) ?>" target="_blank">ดูไฟล์</a>
                    <?php
                    }
                    ?>
                </div>
