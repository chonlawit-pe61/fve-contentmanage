...
<div class="card shadow shadow-lg">
    <form action="<?= base_url('admin/publish/savePublishRepository') ?>" method="POST" autocomplete="off" enctype="multipart/form-data" class="once-only">
        <!-- Hidden -->
        <input type="hidden" name="id" value="<?= (@$publish_repository['id']) ? $publish_repository['id'] : '' ?>">
        <div class="card-body">
            <div class="row">
            ...
            <div class="row justify-content-center my-2">
                <div class="col-md-3 my-2">
                    ...
                    <select name="type_id" id="type_id" class="form-select" required>
                        ...
                        <?php foreach ($publish_repository_types as $type) { ?>
                            <option value="<?= $type['id'] ?>" <?= (@$publish_repository['type_id'] == $type['id']) ? 'selected' : '' ?>><?= $type['name'] ?></option>
                        <?php } ?>
                    </select>
                </div>
                <div class="col-md-7 my-2">
                    <label for="title" class="form-label">หัวข้อ <span class="text-danger">*</span></label>
                    <input type="text" id="title" name="title" class="form-control" value="<?= @$publish_repository['title'] ?>" required>
                </div>
            ...
            <div class="row justify-content-center my-2">
                <div class="col-12 text-center">
                    ...
                    <a href="<?= base_url('admin/publish/publish_repository') ?>" class="btn btn-warning mx-1">
    ...
    function deleteFile(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    method: "POST",
                    url: "<?= base_url('admin/publish/delete-publish-repository-file') ?>",
                    data: {
                        id: id
                    },
                });
