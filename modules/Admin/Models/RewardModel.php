    ...
    function getRewardTables()
    {
        ...
        return DataTable::of($builder)
            ->edit('image_path', function ($row, $meta) {
                if (!empty($row->image_path)) {
                    if (file_exists('./' . $row->image_path)) {
                        ...
                        $img_link = base_url('public/img/trophy.avif');
                    }
                } else {
                    $img_link = base_url('public/img/trophy.avif');
                }
            ...
            ->add('action', function ($row) {
                $tool = '';
                $editLink = base_url('admin/reward/manage/' . $row->id);
                $tool .= <<<HTML
                    <a href="$editLink" class="btn btn-warning btn-sm"><i class="ti ti-pencil"></i></a>
                    <button type="button" class="btn btn-danger btn-sm" onclick="deleteReward('$row->id')"><i class="ti ti-trash"></i></button>
                HTML;
                return $tool;
            })
